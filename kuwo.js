...
/**
 * 获取推荐歌单标签
 * @returns {Promise<Object>} 推荐歌单标签数据
 */
async function getRecommendSheetTags() {
    console.log(`[小蜗音乐] 获取推荐歌单标签`);
    try {
        ...
        console.log(`[小蜗音乐] 获取推荐歌单标签成功: 共 ${res.length} 个标签组`);
        ...
        console.log(`[小蜗音乐] 获取推荐歌单标签返回: 已格式化标签数据`);
        return {
            data,
            pinned,
        };
    } catch (error) {
        console.error(`[小蜗音乐] 获取推荐歌单标签失败: ${error.message}`);
        throw error;
    }
}
...
async function getMediaSource(musicItem, quality) {
    const res = (
        await axios_1.default.get(`https://music-api.gdstudio.xyz/api.php?types=url&source=kuwo&id=${musicItem.id}&br=${qualityLevels[quality]}`, {headers: { },})).data;
    return {
        url: res.url,
    };
}
